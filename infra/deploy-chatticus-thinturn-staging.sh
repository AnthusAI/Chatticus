#!/bin/sh
# Deploy only the staging thin-turn stack. Never --all. Never snapshots or computers.
set -eu

cd "$(dirname "$0")"

if [ "${1:-}" != "" ]; then
  echo "usage: sh deploy-chatticus-thinturn-staging.sh" >&2
  echo "Refuses extra arguments so this cannot be used for development or production." >&2
  exit 2
fi
...
if ! aws sts get-caller-identity >/dev/null; then
  echo "aws login required before ChatticusThinTurnStaging deploy." >&2
  exit 1
fi
...
if CLUSTER="$(read_stack_output ComputerClusterName)" && [ -n "${CLUSTER}" ]; then
  ...
  if [ -n "${TASK_DEF}" ] && [ -n "${SERVICE}" ]; then
    ...
    if [ -n "${NETWORK_JSON}" ] && [ "${NETWORK_JSON}" != "null" ]; then
      SUBNETS="$(
      ...
      if [ -n "${ROLES_JSON}" ] && [ "${ROLES_JSON}" != "null" ]; then
        EXECUTION_ROLE_ARN="$(
...
npx cdk deploy ChatticusThinTurnStaging --exclusively --require-approval never ${CDK_CONTEXT} ${BUDGETS_CDK_CONTEXT}
