...
mkdir -p /results/raw
do_mount
echo "Benchmark ${TAG} on $(hostname)"
...
# Cold pass (not used for verdict)
drop_caches
run_ops "$REPO" "$PIP_DEST" >/dev/null || true

declare -a gs cs ss ps
for i in $(seq 1 "$RUNS"); do
    before="/results/raw/${TAG}-run${i}-before.nfsstat"
    after="/results/raw/${TAG}-run${i}-after.nfsstat"
    nfsstat_snapshot >"$before"
    read -r g c s p <<<"$(run_ops "$REPO" "$PIP_DEST")"
    nfsstat_snapshot >"$after"
    diff_nfsstat "$before" "$after" "/results/raw/${TAG}-run${i}-nfsstat-diff.txt"
    gs+=("$g")
    cs+=("$c")
    ss+=("$s")
    ps+=("$p")
